...
export default function DataBarang() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://akataraforecast.pythonanywhere.com/api/barang');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modeEdit) {
        await axios.put(`https://akataraforecast.pythonanywhere.com/api/barang/${editKode}`, barangPayload);
        console.log('Barang berhasil diupdate.');
      } else {
        await axios.post('https://akataraforecast.pythonanywhere.com/api/barang', barangPayload);
  ...
  const handleDelete = async (kode) => {
    try {
      await axios.delete(`https://akataraforecast.pythonanywhere.com/api/barang/${kode}`);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await axios.post('https://akataraforecast.pythonanywhere.com/api/upload-barang', formData, {
        headers: {
      ...
      >
        <div>
          ...
          <nav className="mt-4 px-4 flex flex-col gap-3">
            ...
            <button onClick={() => navigate('/data-barang')} className="flex items-center gap-2 px-3 py-2 rounded bg-gray-200 text-blue-900">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="text-gray-200 bg-blue-900 h-20 px-6 flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold uppercase tracking-wide">Data Barang</h1>
          </div>
        </header>
        ...
        <main className="p-6 bg-gray-50 flex-1 overflow-y-auto">
          <div className="bg-white p-6 rounded shadow">
            <div className="mb-6 border-b pb-4">
              <h1 className="text-xl font-bold text-gray-700">Manajemen informasi data barang</h1>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-2 md:space-y-0">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => { setShowModal(true); setModeEdit(false); setFormBarang({ kode: '', nama: '', kategori: '', harga: '' }); }} className="flex items-center gap-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 text-sm">
                  <Plus size={16} /> Tambah Barang
                </button>
                <button onClick={() => setShowUploadModal(true)} className="flex items-center gap-2 bg-emerald-500 text-white px-4 py-2 rounded hover:bg-emerald-600 text-sm">
                ...
                <table className="min-w-full text-sm text-gray-700 border">
                  ...
                  <tbody>
                      ...
                      <tr key={barang.kode} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border">
                          <div className="flex gap-2 justify-center">
                            <button onClick={() => handleEdit(barang)} className="bg-yellow-400 text-white p-1 rounded-full hover:bg-yellow-500 transition-colors">
                              <Edit2 size={14} />
                            </button>
                            <button onClick={() => openConfirmModal(barang.kode)} className="bg-rose-500 text-white p-1 rounded-full hover:bg-rose-600 transition-colors">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </td>
                    ...
                    {filteredData.length === 0 && !loading && (
                      <tr>
                        <td colSpan={6} className="text-center py-4 text-gray-400">Tidak ada data barang yang cocok dengan pencarian.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-full max-w-md shadow">
            <h2 className="text-lg font-bold mb-4">{modeEdit ? 'Edit Barang' : 'Tambah Barang'}</h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              <input type="text" name="kode" value={formBarang.kode} onChange={handleInputChange} placeholder="Kode Barang" className="w-full border border-gray-300 px-3 py-2 rounded" required readOnly={modeEdit} />
              <input type="text" name="nama" value={formBarang.nama} onChange={handleInputChange} placeholder="Nama Barang" className="w-full border border-gray-300 px-3 py-2 rounded" required />
              <input type="text" name="kategori" value={formBarang.kategori} onChange={handleInputChange} placeholder="Kategori" className="w-full border border-gray-300 px-3 py-2 rounded" required />
              <input type="text" name="harga" value={formBarang.harga} onChange={handleInputChange} placeholder="Harga Barang (Rp)" className="w-full border border-gray-300 px-3 py-2 rounded" required />
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => { setShowModal(false); setModeEdit(false); }} className="px-4 py-2 rounded bg-gray-300 text-gray-800">Batal</button>
                <button type="submit" className="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Simpan</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-full max-w-sm shadow">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={closeConfirmModal} className="px-4 py-2 rounded bg-gray-300 text-gray-800">Tidak</button>
              <button onClick={() => handleDelete(itemToDelete)} className="px-4 py-2 rounded bg-rose-600 text-white hover:bg-rose-700">Ya, Hapus</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-full max-w-sm shadow">
            ...
            <p className="mb-4 text-sm text-gray-600">
              ...
              <code className="bg-gray-200 px-1 rounded mx-1">kategori</code>, dan 
