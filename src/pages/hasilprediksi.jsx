...
import {
    BarChart2, ShoppingCart, LineChart as IconLineChart, LogOut,
    Brain, PieChart, BarChart3Icon, Menu, Maximize, X, Download
} from 'lucide-react';
...
export default function HasilPrediksi() {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            root.render(<PdfContent predictionResult={predictionResult} onRendered={resolveRender} />);
            await renderPromise;
            await new Promise(r => setTimeout(r, 1000));
            const dataUrl = await htmlToImage.toPng(tempDiv, { quality: 1.0, backgroundColor: '#ffffff' });
            ...
            pdf.save(`Hasil_Prediksi_${predictionResult.barang_name || 'Produk'}.pdf`);
        } catch (error) {
    ...
    const historicalValues = historicalDataForChart.map(item => item.total_jumlah);
    ...
    const historicalPredictionValues = combinedTableData.map(item => item.prediksi !== null ? Math.round(item.prediksi) : null);
    ...
    const data = {
        ...
        datasets: [
            ...
            { 
                ...
                data: historicalPredictionValues.concat(Array(forecast.length).fill(null)), 
            ...
            { 
                ...
                data: Array(historicalValues.length - 1).fill(null)
    ...
    const options = {
        ...
        plugins: { 
            legend: { position: 'top' }, 
            title: { display: true, text: 'Perbandingan Data Aktual vs Prediksi DES' } 
        },
        scales: { 
    ...
    const getMapeInterpretation = (mapeValue) => {
        ...
        if (roundedMape <= 0.10) return { text: 'Sangat Baik', color: 'text-green-600' };
        if (roundedMape <= 0.20) return { text: 'Baik', color: 'text-blue-600' };
        if (roundedMape <= 0.50) return { text: 'Cukup', color: 'text-yellow-600' };
            ...
            >
                <div>
                    ...
                    <nav className="mt-4 px-4 flex flex-col gap-3">
                        ...
                        <button onClick={() => navigate('/penjualan')} className="flex items-center gap-2 hover:bg-gray-200 px-3 py-2 rounded">
                ...
                <div className="p-4">
                    <button onClick={handleLogout} className="w-full bg-red-600 text-white py-2 rounded flex items-center justify-center gap-2 hover:bg-red-700">
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
                <header className="main-header text-gray-200 bg-blue-900 h-20 px-6 flex items-center justify-between shadow-md">
                    <div className="flex items-center gap-3">
                        <button className="p-2 hover:bg-gray-300 rounded" onClick={() => setSidebarOpen(!sidebarOpen)}>
                            <Menu size={28} />
                        </button>
                        <h1 className="text-xl font-bold uppercase tracking-wide">Hasil Prediksi</h1>
                    </div>
                </header>
                ...
                <main className="p-6 bg-gray-50 flex-1 overflow-y-auto">
                    ...
                    <div className="mb-6 flex justify-between items-center">
                        <h2 className="text-2xl font-bold text-gray-800">Hasil Prediksi Permintaan Produk {namaBarang}</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <div className="bg-white p-6 rounded-lg shadow-md">
                            <h3 className="text-lg font-semibold text-gray-700 mb-4">Parameter DES</h3>
                            <div className="flex justify-around items-center">
                                <div className="text-center">
                                    <span className="text-4xl font-bold text-blue-600">{optimized_alpha.toFixed(3)}</span>
                                    <p className="text-sm text-gray-500">Alpha (α)</p>
                                </div>
                                <div className="text-center">
                                    <span className="text-3xl font-bold text-blue-600">{optimized_beta.toFixed(3)}</span>
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-md flex flex-col justify-between">
                            <h3 className="text-lg font-semibold text-gray-700">Akurasi Prediksi</h3>
                            <div className="flex-1 flex flex-col items-center justify-center">
                                {/* ✅ PERUBAHAN 3: Hilangkan perkalian *100* karena backend sudah mengalikan 100 */}
                                <span className="text-5xl font-bold text-green-600">{mape.toFixed(2)}%</span>
                                <p className="text-sm text-gray-500">Mean Absolute Percentage Error (MAPE)</p>
                                <p className={`mt-2 font-bold ${mapeInterpretation.color}`}>Status: {mapeInterpretation.text}</p>
                            </div>
                        </div>

                        <div className="bg-white p-6 rounded-lg shadow-md">
                            <h3 className="text-lg font-semibold text-gray-700 mb-4">Interpretasi MAPE</h3>
                            <ul className="space-y-2 text-sm text-gray-600">
                                <li className="flex items-center gap-2"><span className="w-4 h-4 bg-green-200 rounded-full"></span><span className="flex-1">Sangat Baik (&lt; 10%)</span></li>
                                <li className="flex items-center gap-2"><span className="w-4 h-4 bg-blue-200 rounded-full"></span><span className="flex-1">Baik (10% - 20%)</span></li>
                                <li className="flex items-center gap-2"><span className="w-4 h-4 bg-yellow-200 rounded-full"></span><span className="flex-1">Cukup (20% - 50%)</span></li>
                                <li className="flex items-center gap-2"><span className="w-4 h-4 bg-red-200 rounded-full"></span><span className="flex-1">Rendah (&gt; 50%)</span></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-gray-700">Visualisasi Prediksi</h3>
                            <button
                                onClick={toggleChartFullScreen}
                                className="bg-gray-200 hover:bg-gray-300 p-2 rounded-full"
                                aria-label="Perbesar Grafik"
                            >
                                {isChartFullScreen ? <X size={20} className="text-gray-600" /> : <Maximize size={20} className="text-gray-600" />}
                            </button>
                        </div>
                    ...
                    {/* Tabel Prediksi */}
                    <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                        <h3 className="text-xl font-bold mb-4 text-gray-700">Tabel Hasil Prediksi</h3>
                            ...
                            <table className="min-w-full text-sm text-gray-700 border border-collapse">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 border text-center">Hasil Prediksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {forecast && forecast.length > 0 ? (
                                        forecast.map((item, index) => (
                                            <tr key={index} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 border text-center">{Math.round(item.forecast)}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan={3} className="text-center py-4 text-gray-400">Belum ada hasil prediksi.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                    ...
                    {/* Tabel Error */}
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <h3 className="text-xl font-bold mb-4 text-gray-700">Perhitungan Error</h3>
                        <div className="overflow-auto">
                            <table className="min-w-full text-sm text-gray-700 border border-collapse">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {combinedTableData && combinedTableData.length > 0 ? (
                                        combinedTableData.map((item, index) => (
                                            <tr key={index} className="hover:bg-gray-50">
                                                <td className="px-4 py-2 border text-left">{item.bulan}</td>
                                                <td className="px-4 py-2 border text-center">{item.aktual}</td>
                                                <td className="px-4 py-2 border text-center">{item.level !== null ? item.level.toFixed(2) : '-'}</td>
                                                <td className="px-4 py-2 border text-center">{item.trend !== null ? item.trend.toFixed(2) : '-'}</td>
                                                <td className="px-4 py-2 border text-center">{item.prediksi !== null ? item.prediksi.toFixed(2) : '-'}</td>
                                                <td className="px-4 py-2 border text-center">{item.error !== null ? item.error.toFixed(2) : '-'}</td>
                                                <td className="px-4 py-2 border text-center">{item.ape !== null ? item.ape.toFixed(2) + '%' : '-'}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan={7} className="text-center py-4 text-gray-400">Data perhitungan tidak tersedia.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
